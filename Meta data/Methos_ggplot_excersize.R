...
ggplot(Photosurvey, aes(x=season, y=Algae))+
  geom_point()+
  theme_light()
...
ggplot(merged_data, aes(x = Algae, y = Bryozoa)) +
  geom_point() +
  ggtitle("Correlation between Algae and Bryozoa Coverage") +
  xlab("Algae Coverage (%)") +
  ylab("Bryozoa Coverage (%)")
...
##### scatter plots #####
ggplopt(data = alaska_flights, mapping = aes(x=dep_delay, y=arr_delay)) +
  geom_point()
...
ggplot(data = early_january_weather, mapping = aes(x=time_hour, y=temp)) +
  geom_line(lty=1, lwd=1, color = "25")
...
ggplot(weather, aes(temp)) +
  geom_histogram(binwidth = 10, color = "white")
  facet_wrap(-month, row = 4)
  
  ##### Boxplots #####
  ggplot(weather, aes(month, temp))+
    geom_boxplot()
  ...
  ##### Bar plots #####
  fruits <- tibble(fruit = c('apple', 'apple', 'orange', 'orange', 'apple'))
  View(fruits)
  
  fruits_counted <- tibble(fruit = c('apple', 'orange', number=c(3,2)))
                           View(fruits_counted)
  
  ggplot(fruits_counted, aes(fruit, number)) +
    geom_col()
  
  ggplot(flights, aes(carrier, fill = origin)) +
    geom_bar()
  
  ggplot(flights, aes(carrier, color = origin)) +
    geom_bar()
